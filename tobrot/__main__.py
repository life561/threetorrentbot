...
if __name__ == "__main__" :
    ...
    incoming_message_handler = MessageHandler(
        incoming_message_f,
        filters=Filters.command(["leech@Threetorrentbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    incoming_youtube_dl_handler = MessageHandler(
        ...
        filters=Filters.command(["ytdl@Threetorrentbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@Threetorrentbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["savethumbnail@Threetorrentbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command(["clearthumbnail@Threetorrentbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
